...
var asdElementList'.$list->table_id.' = new AsdIblockElementList('.CUtil::PhpToJSObject($jsParams, false, true, true).');
        ...
        if ($bMixPage || $strCurPage === '/bitrix/admin/iblock_element_admin.php' ||
            ...
            $list->context->additional_items[] = [
                'TEXT' => GetMessage('ASD_IBLOCK_SETT_SECT_MODE'),
                'TITLE' => GetMessage('ASD_IBLOCK_SETT_SECT_MODE_TITLE'),
                'GLOBAL_ICON' => 'adm-menu-setting',
                'ONCLICK' => "location.href='".htmlspecialcharsbx($GLOBALS['APPLICATION']->GetCurPageParam('action=asd_reverse&'.bitrix_sessid_get(),
                        ['action']))."'",
            ];
        }
        ...
        if ($bRightPage && CModule::IncludeModule('iblock')) {
            ...
            $find_section = (int)$_REQUEST['find_section_section'];
            if ($find_section < 0) {
                $find_section = 0;
            }
            ...
            if (CModule::IncludeModule('catalog')) {
                $productIBlock = CCatalog::GetList([], ['PRODUCT_IBLOCK_ID' => $IBLOCK_ID], false, false,
                    ['IBLOCK_ID'])->Fetch();
            ...
            while ($arIB = $rsIB->GetNext(true, false)) {
                ...
                if (!isset($arIBblocks[$arIB['IBLOCK_TYPE_ID']])) {
                    $arIBblocks[$arIB['IBLOCK_TYPE_ID']] = [
                        'NAME' => $arIBtypes[$arIB['IBLOCK_TYPE_ID']],
                        'ITEMS' => [],
                    ];
                }
                $arIBblocks[$arIB['IBLOCK_TYPE_ID']]['ITEMS'][] = ['ID' => $arIB['ID'], 'NAME' => $arIB['NAME']];
            }
            ...
            if (CASDiblockVersion::isIblockNewGridv18()) {
                ...
                if ($boolAccess) {
                    ...
                    $list->arActions['asd_copy_move'] = ['type' => 'html', 'value' => $strIBlocksCp];
                    $list->arActions['asd_copy_move_sect'] = ['type' => 'html', 'value' => $strSectionSelect];
...
var asdElementList'.$list->table_id.' = new AsdIblockElementList('.CUtil::PhpToJSObject($jsParams, false, true, true).');
    ...
    public static function OnAdminContextMenuShowHandler(&$items)
    {
        if ($GLOBALS['APPLICATION']->GetCurPage() == '/bitrix/admin/iblock_edit.php' && $_REQUEST['ID'] > 0) {
            ...
            $importAction = "javascript:(new BX.CDialog({
							...
							content: '<form action=\"".CUtil::JSEscape($GLOBALS['APPLICATION']->GetCurPageParam('',
            ...
            $items[] = [
                ...
                'MENU' => [
                    [
                        ...
                        'ACTION' => version_compare(SM_VERSION,
                            '11.5.5') >= 0 ? $exportAction : htmlspecialcharsbx($exportAction),
                    ],
                    [
                        'TEXT' => GetMessage('ASD_ACTION_IMPORT_PROP'),
                        'ACTION' => version_compare(SM_VERSION,
                            '11.5.5') >= 0 ? $importAction : htmlspecialcharsbx($importAction),
                    ],
                ],
            ];
        }
    ...
    public static function OnAdminTabControlBeginHandler(&$form)
    {
        ...
        if (!$bPublicLinkShow && array_key_exists('ID',
                $_REQUEST) && intval($_REQUEST['ID']) > 0 && $_REQUEST['bxpublic'] == 'Y' &&
          ...
          <div style="float: right">
            <a style="text-decoration: none;"
               href="/bitrix/admin/iblock_element_edit.php?ID=<?= intval($_REQUEST['ID']) ?>&amp;<?
